...
//  char messagename1[20]={'收'};
  ...
  wTime = (char *)GUI_VMEM_Alloc(1024);
        ...
        case BN_CLICKED:
        {
          switch (id)
          {
            ...
            case eID_SMS_SEND:
            {
              ...
              cContent = (char  *)GUI_VMEM_Alloc(1024);
              wNumber  = (WCHAR *)GUI_VMEM_Alloc(200*sizeof(WCHAR));
              wContent = (WCHAR *)GUI_VMEM_Alloc(1024*sizeof(WCHAR));
              
              GetWindowText(GetDlgItem(hwnd, eID_SMS_NUMBER), wNumber, 99);
              GetWindowText(GetDlgItem(hwnd, eID_SMS_CONTENT), wContent, 1024);
              x_wcstombs(cNumber, wNumber, 99);
              x_wcstombs(cContent, wContent, 1024);
              printf("-----------%s\r\n",cNumber);
            ...
            case eID_SMS_LIST:
            {
              ...
              cContent = (char  *)GUI_VMEM_Alloc(1024);
              wNumber  = (WCHAR *)GUI_VMEM_Alloc(200*sizeof(WCHAR));
              wContent = (WCHAR *)GUI_VMEM_Alloc(768*sizeof(WCHAR));
              
              /* 读短信内容 */
              if (readmessage(ListData & 0x7FFF, cNumber, cContent))
              {
                hexuni2gbk((char *)cNumber, (char *)&cNumber[100]);	
                hexuni2gbk((char *)cContent, (char *)&cContent[384]);
...
//                GUI_DEBUG("cContent:(%s)", (char *)&cContent[2000]);
                ...
                x_mbstowcs(wContent, (char *)&cContent[384], 384);	// 将Ansi字符转换成GUI的unicode字符.
