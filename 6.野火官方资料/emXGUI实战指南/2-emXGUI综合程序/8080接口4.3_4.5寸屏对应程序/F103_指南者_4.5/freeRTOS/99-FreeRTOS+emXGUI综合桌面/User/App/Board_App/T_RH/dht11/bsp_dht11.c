...
//#define DHT11_DELAY_MS(ms)  GUI_msleep(ms)
...
static uint8_t DHT11_ReadByte ( void )
{
	...
	{	 
		...
		{
			/* 等待数据1的高电平结束 */
			while(DHT11_Dout_IN()==Bit_SET){count--;if(count==0){count=0xfff;break;}}
...
{  
	...
	if(DHT11_Dout_IN()==Bit_RESET)     
	{
		...
		while(DHT11_Dout_IN()==Bit_RESET){count--;if(count==0){count=0xfff;break;}}
