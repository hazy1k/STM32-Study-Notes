...
extern const unsigned char acbmp_index8[];
extern const unsigned char acbmp_rgb565[];
extern const unsigned char acbmp_rgb888[];

#define test_bmp acbmp_rgb565
	...
	switch(msg)
	{
		...
		case WM_PAINT: //窗口需要绘制时，会自动产生该消息.
		{
			...
			if(step==1)
			{
				...
				BMP_Draw(hdc,r.x,r.y,test_bmp,NULL);
			...
			if(step==2)
			{
				...
				x_wsprintf(wbuf,L"BMP-24bit Speed\r\n"
...
void	GUI_DEMO_BMP_Speed(void)
{
	...
	hwnd	=CreateWindowEx(	WS_EX_LOCKPOS,
								&wcex,
								_T("emXGUI DEMO - 24bit BMP Speed"), //窗口名称
