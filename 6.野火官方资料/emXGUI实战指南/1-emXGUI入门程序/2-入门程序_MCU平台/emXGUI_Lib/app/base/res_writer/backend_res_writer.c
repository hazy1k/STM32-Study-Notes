...
FRESULT BurnFile(void)
{
  ...
  /* 初始化各种全局变量 */
  {
   src_dir =  (char *)GUI_MEM_Alloc(512);
   full_file_name =  (char *)GUI_MEM_Alloc(512);
   line_temp =  (char *)GUI_MEM_Alloc(512);
   fp = (FIL *)GUI_MEM_Alloc(sizeof(FIL));
  }
...
exit:
  /* 释放申请的空间 */
  {
    GUI_MEM_Free(src_dir);
    GUI_MEM_Free(full_file_name);
    GUI_MEM_Free(line_temp);
    GUI_MEM_Free(fp);
  }
