...
BMP图片显示支持两种图片显示方式：一种是从外部存储器中读取数据并显示，一边读一边显示，占用 RAM 空间小，但显示速度很慢
另一种是从外部存储器把图片数据全部读取出来再显示，显示速度较快，但占用大量 RAM空间
实验现象：本实验中是从 SD 卡加载，可以在 LCD 上看到GUI_BMP_DrawEx 函数的 BMP 有很明显的从下到上的显示过程
