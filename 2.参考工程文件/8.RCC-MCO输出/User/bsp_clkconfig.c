...
// 设置系统时钟，使用HSE作为时钟源，PLL倍频因子为9
void HSE_SetSysClock(uint32_t pllmul)
{
  // __IO代表可读写的变量，volatile代表易变的变量，防止编译器优化
	__IO uint32_t StartUpCounter = 0, HSEStartUpStatus = 0; // 等待HSE启动的计数器和HSE启动状态标志
...
//-----------------设置各种频率主要就是在这里设置-------------------//
    // 设置PLL时钟来源为HSE，设置PLL倍频因子
		// PLLCLK = 8MHz * pllmul
		RCC_PLLConfig(RCC_PLLSource_HSE_Div1, pllmul); // 使用库函数 RCC_PLLConfig()
//------------------------------------------------------------------//
