...
void AD7192_Init(void)
{
  ...
  /* 使能SPI外设以及SPI引脚时钟 */
	...
	AD7192_CS_CLK_ENABLE();
	AD7192_SCK_CLK_ENABLE();
	AD7192_MISO_CLK_ENABLE();
	AD7192_MOSI_CLK_ENABLE();
	
  GPIO_InitStruct.Pin = AD7192_SCK_Pin;
  ...
  HAL_GPIO_Init(AD7192_MISO_GPIO_Port, &GPIO_InitStruct);
	...
	GPIO_InitStruct.Pin = AD7192_MOSI_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
  HAL_GPIO_Init(AD7192_MOSI_GPIO_Port, &GPIO_InitStruct);
...
{
  	unsigned	char	WriteData, ReadData;     //分别定义一个要写出数据的中间变量和一个要读入数据的中间变量
  	unsigned	char	i, j;
	  AD7192_SCLK_H;//时钟拉高
/*-------------------------------------外循环执行判断是否达到要发送的字节数-------------------------------------------*/
	for(i=0; i<NumberOfByte; i++)
 	{
	 	WriteData = *(WriteBuffer + i);         //写缓冲器里面的数据赋给中间变量WriteData
		ReadData = 0;
      /*---------------------------------内循环执行完毕发送和接收8位数据----------------------------------------*/
		for(j=0; j<8; j++)
		{
      ...
      //Delay(0x0FFF); 
			...
			{
				HAL_GPIO_WritePin(AD7192_MOSI_GPIO_Port, AD7192_MOSI_Pin,GPIO_PIN_SET);	      //Send one to SDI pin   发送1至SDI引脚
			}
			else
			{
				HAL_GPIO_WritePin(AD7192_MOSI_GPIO_Port, AD7192_MOSI_Pin,GPIO_PIN_RESET);	      //Send zero to SDI pin  发送0至SDI引脚
			}
			WriteData = WriteData << 1;
 
			 AD7192_SCLK_L;         //时钟拉低，时钟拉高要干读取的事
    
			 ReadData = (ReadData<<1) | HAL_GPIO_ReadPin(AD7192_MISO_GPIO_Port, AD7192_MISO_Pin); //将IO口接收到的高或低电平保存至ReadData。
		}
    /*---------------------------------------内循环结束-----------------------------------------------------*/
		*(ReadBuffer + i)= ReadData;
	}  //读写操作完成，将WriteBuffer里的值通过中间变量WriteData借助ADuC7026OutputBit(SDI,0)或ADuC7026OutputBit(SDI,0)发送出去。
