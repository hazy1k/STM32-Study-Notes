...
void AD7192_Init(void)
{
  ...
  /* 使能SPI外设以及SPI引脚时钟 */
	/* 使能SPI时钟 */
	AD7192_SPI_APBxClock_FUN ( AD7192_SPI_CLK, ENABLE );
	
	/* 使能SPI引脚相关的时钟 */
 	AD7192_SPI_CS_APBxClock_FUN ( AD7192_SPI_CS_CLK|AD7192_SPI_SCK_CLK|
																	AD7192_SPI_MISO_PIN|AD7192_SPI_MOSI_PIN, ENABLE );
	/* 配置SPI的 CS引脚，普通IO即可 */
...
void SPI_TransmitReceive(unsigned char* WriteBuffer, unsigned char *ReadBuffer, unsigned char NumberOfByte,uint32_t  SPITimeout)
{
	unsigned char i;
	...
	for(i = 0;i < NumberOfByte; i++)
	{
			...
			{
				if((SPITimeout--) == 0) {SPI_TIMEOUT_UserCallback(0); return;}
			 }

			/* 写入数据寄存器，把要写入的数据写入发送缓冲区 */
			SPI_I2S_SendData(AD7192_SPIx , WriteBuffer[i]);

			SPITimeout = Timeout;
			/* 等待接收缓冲区非空，RXNE事件 */
			while (SPI_I2S_GetFlagStatus(AD7192_SPIx , SPI_I2S_FLAG_RXNE) == RESET)
			{
				if((SPITimeout--) == 0) {SPI_TIMEOUT_UserCallback(1); return;}
		  }
			
			/* 读取数据寄存器，获取接收缓冲区数据 */
			ReadBuffer[i] = SPI_I2S_ReceiveData(AD7192_SPIx);
	}
...
{
  SPI_TransmitReceive(WriteBuffer, ReadBuffer, NumberOfByte, 1000);	
}
...
uint32_t AD7192ReadConvertingData(void)
{
	...
	while(GPIO_ReadInputDataBit(AD7192_SPI_MISO_PORT,AD7192_SPI_MISO_PIN)== 0){;}			
	while(GPIO_ReadInputDataBit(AD7192_SPI_MISO_PORT,AD7192_SPI_MISO_PIN)== 1){;}			//	waiting the 1st RDY failling edge;即DOUT变为低电平。
