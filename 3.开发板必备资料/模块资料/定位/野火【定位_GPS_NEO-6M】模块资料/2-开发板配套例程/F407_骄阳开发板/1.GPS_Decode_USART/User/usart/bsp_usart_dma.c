...
void USART_DMA_Config(void)
{
  ...
  DMA_Handle.Instance = GPS_USART_DMA_STREAM;
	/* DMA数据流通道选择 */
	DMA_Handle.Init .Channel = GPS_DMA_CHANNEL;
  /*方向：从内存到外设*/		
...
void GPS_ProcessDMAIRQ(void)
{
	...
	if(__HAL_DMA_GET_FLAG(&DMA_Handle,DMA_FLAG_HTIF2_6) != RESET)        /* DMA 半传输完成 */
  { 
    GPS_HalfTransferEnd = 1;                //设置半传输完成标志位
		LED1_ON;
  }
  else if(__HAL_DMA_GET_FLAG(&DMA_Handle,DMA_FLAG_TCIF2_6)!= RESET)     /* DMA 传输完成 */
  {
     LED1_OFF;
