...
{								 
	SPI_SetSpeed(SPI_BaudRatePrescaler_64);//设置到低速模式 
}

void VS_SPI_SpeedHigh(void)
{						  
	SPI_SetSpeed(SPI_BaudRatePrescaler_16);//设置到高速模式		 
}
//初始化VS1053的IO口	 
void VS_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(VS_SPIGPIO_CLK|VS_GPIO_DREQ_CLK|VS_GPIO_RST_CLK |VS_GPIO_XDCS_CLK ,ENABLE);
...
// 	GPIO_Init(VS_GPIO_DREQ_PORT, &GPIO_InitStructure);		
  ...
  //设置引脚复用
  GPIO_PinAFConfig(VS_SPIGPIO_PORT,VS_SCLK_PINSOURCE,VS_SPI_AF); 
	GPIO_PinAFConfig(VS_SPIGPIO_PORT,VS_MISO_PINSOURCE,VS_SPI_AF); 
	GPIO_PinAFConfig(VS_SPIGPIO_PORT,VS_MOSI_PINSOURCE,VS_SPI_AF); 
