...
{		
	...
	for(int i=0;i<8;i++)
	{
		...
		if(0x80 == (data & 0x80))
		{
			VS_MOSI_1;   //Send one to SDI pin   发送1至SDI引脚
		}
		else
		{
			VS_MOSI_0;     //Send zero to SDI pin  发送0至SDI引脚
		}
...
void VS_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = VS_SCLK;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(VS_SPIGPIO_PORT, &GPIO_InitStructure);
  
  GPIO_InitStructure.GPIO_Pin = VS_MISO;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(VS_SPIGPIO_PORT, &GPIO_InitStructure);
  
	GPIO_InitStructure.GPIO_Pin = VS_MOSI;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(VS_SPIGPIO_PORT, &GPIO_InitStructure);
  ...
  GPIO_Init(VS_SPIGPIO_PORT, &GPIO_InitStructure); 
	...
	/****其他功能引脚初始化****/
 	...
 	GPIO_InitStructure.GPIO_Pin = VS_RST;	 //PE12
