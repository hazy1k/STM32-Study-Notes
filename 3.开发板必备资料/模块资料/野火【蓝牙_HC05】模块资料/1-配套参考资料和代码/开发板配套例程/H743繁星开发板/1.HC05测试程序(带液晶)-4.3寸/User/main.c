...
{   
  ...
  NT35510_GramScan ( 6 );
	...
	HC05_INFO("**********HC05模块实验************");
	NT35510_DispStringLine_EN_CH (1,(uint8_t*) "HC05 BlueTooth Demo" );

	
	if(HC05_Init() == 0)
	{
		HC05_INFO("HC05模块检测正常。");
		NT35510_DispStringLine_EN_CH ( 2,(uint8_t*) "HC05 module detected!" );

	}
	else
	{
		HC05_ERROR("HC05模块检测不正常，请检查模块与开发板的连接，然后复位开发板重新测试。");
		NT35510_DispStringLine_EN_CH ( 3, (uint8_t*)"No HC05 module detected!"  );
		NT35510_DispStringLine_EN_CH ( 4,(uint8_t*) "Please check the hardware connection and reset the system." );
	...
	NT35510_DispStringLine_EN_CH( 5,(uint8_t*) disp_buff );
	...
	while(1)
	{
		...
		if(Task_Delay[2]==0 && !IS_HC05_CONNECTED() ) 
		{
			if(hc05_role == 1)	//主模式
			{
				...
				if(color==0)	
				{
					NT35510_DispStringLine_EN_CH ( 6,(uint8_t*)"Scaning bluetooth device..." );
					color=1;
				}
				else
				{
		...
		if(Task_Delay[0]==0 && IS_HC05_CONNECTED())  
		{
				...
				NT35510_DispStringLine_EN_CH(7,(uint8_t*)"Bluetooth connected!"  );
				...
				if(linelen<200 && linelen != 0)
				{
					...
					if(strcmp(redata,"AT+LED1=ON")==0)
					{
						...
						NT35510_DispStringLine_EN_CH (8, (uint8_t*)"receive cmd: AT+LED1=ON" );
					}
					else if(strcmp(redata,"AT+LED1=OFF")==0)
					{
						...
						NT35510_DispStringLine_EN_CH (9, (uint8_t*)"receive cmd: AT+LED1=OFF" );
					}
					else
					{
						...
						NT35510_DispStringLine_EN_CH( 10,(uint8_t*)"receive data:" );

						NT35510_DispStringLine_EN_CH( 11,(uint8_t*)linebuff );
			...
			{						
					...
					{				
						...
						if(HC05_Send_CMD(hc05_nameCMD,1) == 0)
						{
							...
							NT35510_DispStringLine_EN_CH( 12,(uint8_t*)disp_buff );
						}
						else
						{							
							HC05_ERROR("更改名字失败");
							
							NT35510_DispStringLine_EN_CH( 12,(uint8_t*)"Rename fail!" );
						}
			...
			else
			{
				...
				if(HC05_Send_CMD("AT+ROLE=1\r\n",1) == 0)	
				{
					...
					if(HC05_Send_CMD(hc05_nameCMD,1) == 0)
					{
						...
						NT35510_DispStringLine_EN_CH(13,(uint8_t*)disp_buff );
					...
					else
					{
						...
						NT35510_DispStringLine_EN_CH( 13,(uint8_t*)"Rename fail!" );
					}
		...
		if( Key_Scan(KEY2_GPIO_PORT,KEY2_PIN) == KEY_ON  )
		{
				...
				if(HC05_Send_CMD(hc05_nameCMD,1) == 0)
				{
					...
					NT35510_DispStringLine_EN_CH( 14,(uint8_t*)disp_buff );
				...
				else
				{
					...
					NT35510_DispStringLine_EN_CH(14,(uint8_t*)"Rename fail!"  );
				}
