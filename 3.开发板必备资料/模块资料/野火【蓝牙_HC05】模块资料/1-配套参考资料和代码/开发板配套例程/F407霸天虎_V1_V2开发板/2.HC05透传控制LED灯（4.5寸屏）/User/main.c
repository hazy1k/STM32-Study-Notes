...
int main(void)
{
  ...
  /* HC05蓝牙模块初始化：GPIO 和 USART3 配置模式为 38400 8-N-1 接收中断 */
	...
	#ifdef ENABLE_LCD_DISPLAY
	ILI9806G_DispStringLine_EN ( (LINE(4)), "ReceiveData USART1" );	
  ILI9806G_DispStringLine_EN ( (LINE(12)), "ReceiveData HC-05" );
  #endif
...
void TransData_CtrlLED_Test(void)
{
  ...
  if(DEBUG_USART_ReceiveData.receive_data_flag == 1)
  {
    ...
    ILI9806G_DispStringLine_EN ( (LINE(5)), (char *)DEBUG_USART_ReceiveData.uart_buff );
    #endif
  ...
  if(BLT_USART_ReceiveData.receive_data_flag == 1)
  {
    ...
    ILI9806G_DispStringLine_EN ( (LINE(13)), (char *)BLT_USART_ReceiveData.uart_buff );
    #endif
