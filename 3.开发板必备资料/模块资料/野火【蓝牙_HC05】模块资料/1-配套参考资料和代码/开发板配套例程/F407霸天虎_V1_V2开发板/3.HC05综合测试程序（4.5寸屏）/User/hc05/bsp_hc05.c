...
void printBLTInfo(void)  
{
  ...
  ILI9806G_Clear(0,160,480,640);
  ILI9806G_DispString_EN( 50, 160,disp_buff );
  #endif
	...
	else
	{
		...
		for(i=0;i<bltDevList.num;i++)
		{
			...
			ILI9806G_DispString_EN( 5, 200+i*100,disp_buff );
			
			sprintf(disp_buff,"Addr: %s",bltDevList.unpraseAddr[i]);
			ILI9806G_DispString_EN( 5, 230+i*100,disp_buff );
			
			sprintf(disp_buff,"Name: %s",bltDevList.name[i]);
			ILI9806G_DispString_EN( 5, 260+i*100,disp_buff );
      #endif
		}
	}
...
uint8_t linkHC05(void)
{
  ...
  parseBltAddr();         //查询蓝牙设备并解析地址
	...
	for(i=0;i<=bltDevList.num;i++)
	{
      ...
      #ifdef ENABLE_LCD_DISPLAY
			LCD_SetColors(YELLOW,BLACK);
			ILI9806G_Clear(0,80,240,20);
			ILI9806G_DispString_EN( 5, 80,"Found a HC05 ,conecting..." );
			#endif
