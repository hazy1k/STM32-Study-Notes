...
void printBLTInfo(void)  
{
  ...
  LCD_DisplayStringLine( LCD_LINE_5,(uint8_t *)disp_buff );
  #endif
	...
	else
	{
		...
		for(i=0;i<bltDevList.num;i++)
		{
			...
			LCD_ClearLine( LINE(7+i*3+0) );
			LCD_DisplayStringLine( LINE(7+i*3+0),(uint8_t *)disp_buff );

			sprintf(disp_buff,"Addr: %s  Name: %s", bltDevList.unpraseAddr[i], bltDevList.name[i]);
			LCD_ClearLine(LINE(7+i*3+1));
			LCD_DisplayStringLine( LINE(7+i*3+1),(uint8_t *)disp_buff );
      #endif
		}
	}
  ...
  parseBltAddr();         //查询蓝牙设备并解析地址
	...
	for(i=0;i<=bltDevList.num;i++)
	{
		if(strstr(bltDevList.name[i],"HC05") != NULL) //非NULL表示找到有名称部分为HC05的设备
		{
			...
			LCD_DisplayStringLine( LCD_LINE_5,(uint8_t *)"Found a HC05 ,conecting..." );
			#endif
