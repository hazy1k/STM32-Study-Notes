...
int main(void)
{
  ...
  /* HC05蓝牙模块初始化：GPIO 和 USART2 配置模式为 38400 8-N-1 接收中断 */
	if(HC05_Init() == 0)
	{
  ...
  HC05_INFO("**********HC05模块实验************");
	...
	LCD_DisplayStringLine ( (LINE(4)), (uint8_t *)"ReceiveData USART1" );	
  LCD_DisplayStringLine ( (LINE(9)), (uint8_t *)"ReceiveData HC-05" );
  #endif
  ...
  SysTick_Init();
    ...
    if( 1 == hc05_inquery_connect )
    {
      ...
      if( ! IS_HC05_CONNECTED() )
      {
        ...
        Usart_SendString( DEBUG_USART, (uint8_t *)"蓝牙尚未连接。请用手机打开蓝牙调试助手搜索连接蓝牙\r\n" );
      }
      else
      {
        Usart_SendString( DEBUG_USART, (uint8_t *)"蓝牙已连接。发送“RED_LED”可控制翻转LED灯\r\n" );
      }
    }
...
void TransData_CtrlLED_Test(void)
{
  ...
  if(DEBUG_USART_ReceiveData.receive_data_flag == 1)
  {
    ...
    else if( strstr((char *)DEBUG_USART_ReceiveData.uart_buff,"AT") == (char *)DEBUG_USART_ReceiveData.uart_buff )
    {
      BLT_KEY_HIGHT;
      Usart_SendStr_length(BLT_USARTx, DEBUG_USART_ReceiveData.uart_buff, DEBUG_USART_ReceiveData.datanum);
      Usart_SendStr_length(BLT_USARTx,(uint8_t *)"\r\n",2);
      BLT_KEY_LOW;
    }
