...
启用双ADC模式的时候，通过配置ADC_CR1寄存器的DUALMOD[3:0]位，可以有几种不同的模式， 具体见表格 [双ADC模式的各种模式汇总](https://doc.embedfire.com/mcu/stm32/f103zhinanzhe/std/zh/latest/book/ADC.html#id29) ：
...
static void ADCx_Mode_Config(void)
{
    ...
    // 配置 DMA 初始化结构体
    // 外设基址为：ADC 数据寄存器地址
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&( ADCx_1-
    ));
    // 存储器地址
